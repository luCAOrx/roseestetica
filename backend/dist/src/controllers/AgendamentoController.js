...
exports.default = {
    async listarHorariosDisponiveis(request, response) {
        try {
            const { data } = request.query;
            const horariosDisponiveis = await (0, connection_1.default)('agendamentos')
                .from('horarios')
                .where({ data })
                .join('agendamentos', 'horarios.id', 'agendamentos.horario_id')
                .select('agendamentos.id', 'data', 'horario', 'situacao');
    ...
    async listarAgendamentosDoCliente(request, response) {
        try {
            const { page = 1 } = request.query;
            const { cliente_id } = request.params;
            const agendamento = await (0, connection_1.default)('agendamentos')
                .where({ cliente_id })
                .join('horarios', 'horarios.id', '=', 'agendamentos.horario_id')
                .select('agendamentos.id', 'agendamentos.data', 'horarios.horario')
                .orderBy('id', 'desc')
                .limit(5)
                .offset((Number(page) - 1) * 5);
            const countObjeto = (0, connection_1.default)('agendamentos').count();
    ...
    async detalhesDoAgendamento(request, response) {
        try {
            ...
            const agendamento = await (0, connection_1.default)('agendamentos')
                ...
                .join('horarios', 'horarios.id', '=', 'agendamentos.horario_id')
                .select('agendamentos.id', 'agendamentos.data', 'horarios.horario', 'agendamentos.agendado_em', 'agendamentos.remarcado_em');
            const procedimentos = await (0, connection_1.default)('agendamentos_procedimentos')
                .where({ agendamento_id })
                .andWhere('agendamentos.cliente_id', id)
                .join('agendamentos', 'agendamentos.id', '=', 'agendamentos_procedimentos.agendamento_id')
                .join('procedimentos', 'procedimentos.id', '=', 'agendamentos_procedimentos.procedimento_id')
                .select('agendamentos_procedimentos.id', 'procedimentos.procedimento', 'procedimentos.preco', 'agendamentos_procedimentos.procedimento_alterado_em');
    ...
    async agendar(request, response) {
        try {
            ...
            const horarioIndisponivel = await transaction('agendamentos')
                ...
                .select('horario_id')
                .first();
            const dataDeAgora = (0, dayjs_1.default)().format('YYYY/MM/DD');
            ...
            if ((0, dayjs_1.default)(data).isBefore(dataDeAgora)) {
                return response.status(400).json({
                ...
                const agendamentosProcedimentos = procedimento_id.map((procedimento_id) => {
                    return {
                        agendamento_id,
                        procedimento_id
                    };
    ...
    async remarcar(request, response) {
        try {
            ...
            const horarioIndisponivel = await (0, connection_1.default)('agendamentos')
                .where({ horario_id, data })
                .select('horario_id')
                .first();
            const dataDeAgora = (0, dayjs_1.default)().format('YYYY/MM/DD');
            ...
            if ((0, dayjs_1.default)(data).isBefore(dataDeAgora)) {
                ...
                await (0, connection_1.default)('agendamentos')
                    .update({
                    data,
                    horario_id,
    ...
    async alterarProcedimento(request, response) {
        try {
            const { procedimento_id } = request.body;
            const { agendamento_id } = request.params;
            const procedimentos = procedimento_id.map((procedimento_id) => {
                return {
                    agendamento_id,
                    procedimento_id
                };
            ...
            const transaction = await connection_1.default.transaction();
            const dataEhoraDeAgora = new Date();
            const dataDoAgendamentoNoBancoDeDados = await transaction('agendamentos')
                .where({ id: agendamento_id })
                .select('data')
                .first();
            const dataDeAgoraFormatada = (0, dayjs_1.default)().format('YYYY/MM/DD');
            const dataDoAgendamento = (0, dayjs_1.default)(dataDoAgendamentoNoBancoDeDados.data).format('YYYY/MM/DD');
            if ((0, dayjs_1.default)(dataDoAgendamento).isBefore(dataDeAgoraFormatada)) {
                return response.status(400).json({
    ...
    async cancelar(request, response) {
        try {
            const { id } = request.params;
            const dataDoAgendamentoNoBancoDeDados = await (0, connection_1.default)('agendamentos')
                .where({ id })
                .select('data')
                .first();
            const dataDeAgora = (0, dayjs_1.default)().format('YYYY/MM/DD');
            const dataDoAgendamento = (0, dayjs_1.default)(dataDoAgendamentoNoBancoDeDados.data).format('YYYY/MM/DD');
            if ((0, dayjs_1.default)(dataDoAgendamento).isBefore((0, dayjs_1.default)(dataDeAgora))) {
                return response.status(400).json({
                    mensagem: 'O dia para cancelar já passou.'
                });
            }
            else {
                await (0, connection_1.default)('agendamentos').where({ id }).del();
