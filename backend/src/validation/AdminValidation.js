...
module.exports = {
  ...
  cadastrar(request, response, next) {
    ...
    const value = schema.cliente.validate(request.body);
    if (value.error) {
    ...
    const bodySchema = {
      atualizarSenha: Joi.object().keys({
        ...
        token: Joi.string().min(40).max(40).trim().messages({
          ...
          "string.trim": `O campo "Senha" não permite espaços no começo e no fim.`,
          "string.empty": `"Token" não é permitido ficar vazio`,
          "string.min": `"Token" deve ter no mínimo {#limit} caracteres`,
          "string.max": `"Token" deve ter no máximo {#limit} caracteres`,
          "any.required": `"Token" esse campo é obrigatório`
        }).required()
      })
    }
  ...
  // CLIENTE
  cadastrarCliente(request, response, next) {
    const schema = {
      cadastro: Joi.object().keys({
        ...
        email: Joi.string().min(5).max(60).trim().messages({
          ...
          "string.trim": `O campo "E-mail" não permite espaços no começo e no fim.`,
          "string.email": `O campo "E-mail" tem que se parecer um e-mail.`,
          "string.min": `O campo "Usuário" deve ter no mínimo {#limit} caracteres.`,
          "string.max": `O campo "Usuário" deve ter no máximo {#limit} caracteres.`,
          "string.empty": `O campo "E-mail" não pode ficar vazio.`,
          "any.required": `O campo "E-mail" é obrigatório.`
        }).email().required(),
